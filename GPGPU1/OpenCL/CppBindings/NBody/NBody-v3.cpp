...
{
	std::string banner = "Manybody OpenCL v3: N^2, cache unaware, unaliased, minimal calculating";
	...
	try
	{
		...
		std::ifstream source_file(std::string(MANYBODY_OPENCL_KERNEL_PATH) + "manybody-cl-v3-single-device.cl");
        ...
        euler_lws = cl::NDRange(forward_euler.getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(device));

        if (!quiet_arg.getValue())
            std::cout << "Interaction kernel preferred WGS: " << interaction_lws[0] << std::endl;
        if (!quiet_arg.getValue())
            std::cout << "Forward Euler kernel preferred WGS: " << euler_lws[0] << std::endl;
