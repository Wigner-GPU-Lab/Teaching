...
{
	std::string banner = "Manybody OpenCL v4: N^2, cache aware, unaliased, minimal calculating";
	...
	try
	{
		...
		std::ifstream source_file(std::string(MANYBODY_OPENCL_KERNEL_PATH) + "manybody-cl-v4-single-device.cl");
		...
		if (device.getInfo<CL_DEVICE_LOCAL_MEM_SIZE>() < interaction.getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(device) * sizeof(particle))
		{
			interaction_lws = cl::NDRange(256);
			if (!quiet_arg.getValue()) { std::cout << "Throttling interaction WGS to 256 due to insufficient __local memory." << std::endl; }
		}
		else
			interaction_lws = cl::NDRange(interaction.getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(device));

		interaction.setArg( 0, buffer );
		interaction.setArg( 1, cl::Local(interaction_lws[0] * sizeof(particle)) );
	...
	{
		if (!quiet_arg.getValue()) { std::cout << "Computing..."; std::cout.flush(); }
