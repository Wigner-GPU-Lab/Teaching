...
{
	std::string banner = "Manybody v5 parallel: (N^2)/2, cache aware, unaliased, minimal calculating";
...

	{
		...
		for ( std::size_t n = 0; n < iterate_arg.getValue(); ++n )
		{
			...
			{
				*thread = std::async( std::launch::async, [&particles, &threads, &cache_size, &mutexes]( std::ptrdiff_t id )
				{
