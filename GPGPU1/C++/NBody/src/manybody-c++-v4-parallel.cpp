...
{
	std::string banner = "Manybody v4 parallel: N^2, cache aware, unaliased, minimal calculating";
...

	{
		...
		for ( std::size_t n = 0; n < iterate_arg.getValue(); ++n )
		{
			...
			{
				*thread = std::async( std::launch::async, [&particles, &threads, &cache_size]( std::ptrdiff_t id )
				{
					for ( auto cache = particles.begin(); cache != particles.end(); cache += cache_size )
					{
						for ( auto IT = particles.begin() + id * std::distance( particles.begin(), particles.end() ) / threads.size(); IT != particles.begin() + (id + 1) * std::distance( particles.begin(), particles.end() ) / threads.size(); ++IT )
						{
							...
							auto masked_after_IT = mask_range( std::next( IT ), particles.end(), cache, cache + cache_size );
							...
							for ( auto it = masked_after_IT.first; it != masked_after_IT.second; ++it )
							{
								auto f = calculate_force( *IT, *it );
