...
template <typename T>
void matmul_kernel(cl::sycl::queue& deviceQueue, int size, int blocksize, std::vector<T> const& mA, std::vector<T> const& mB, std::vector<T>& mC)
{
	...
	
	{
		deviceQueue.submit([&](cl::sycl::handler &cgh)
		{
			...
			{
				int lx = (int)i.get_local_id(0);
				int ly = (int)i.get_local_id(1);
				
				int gx = (int)i.get_global_id(0);
				int gy = (int)i.get_global_id(1);
...
int main()
{
	...
	try
	{
		...
		std::cout << "Selected platform: " << deviceQueue.get_context().get_platform().get_info<cl::sycl::info::platform::name>() << "\n";
		std::cout << "Selected device:   " << deviceQueue.get_device().get_info<cl::sycl::info::device::name>() << "\n";
